    ...
    for (int i = start; i < end; i++) {
        while (Bo[i] != -1 && sum2 > 0) {
            if (Xo[Bo[i]] != -1) {
                ...
                {
                	t0 = _mm256_loadu_si256((__m256i*)(&B[i*col+j]));
                	t1 = _mm256_loadu_si256((__m256i*)(&X[Xo[Bo[i]]*col+j]));
                	t1 = _mm256_xor_si256(t0, t1);
                	_mm256_storeu_si256((__m256i*)(&B[i*col+j]), t1);//将异或后的结果存回去
                	if (newfirst == -1)
                	{
                		for (int m = 7; m >= 0; m--)
                		{
    ...
    for (int i = start; i < end; i++) {
        while (local_Bo[i] != -1) {
            if (Xo[local_Bo[i]] != -1) {
                ...
                for (int j = d - 8; j >= 0; j -= 8)
                {
                    t0 = _mm256_loadu_si256((__m256i*)(&local_B[i * col + j]));
                    t1 = _mm256_loadu_si256((__m256i*)(&X[Xo[local_Bo[i]] * col + j]));
                    t1 = _mm256_xor_si256(t0, t1);
                    _mm256_storeu_si256((__m256i*)(&local_B[i * col + j]), t1);//将异或后的结果存回去
                    if (newfirst == -1)
                    {
                        for (int m = 7; m >= 0; m--)
                        {
