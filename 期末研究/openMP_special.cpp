...
void MP()
{
#pragma omp parallel for if(parallel), num_threads(thread_num), private(i,d,newfirst)
	for (int i = 0; i < sum3; i++)
	{
		while (Bo[i] != -1 && sum2 > 0)//此被消元行不是全0且实时的被消元行还存在，不为空
		{
...
void MP_SSE()
{
#pragma omp parallel for if(parallel), num_threads(thread_num), private(i,j,d,m,newfirst,t0,t1)
	__m128i t0, t1;//整形寄存器
	for (int i = 0; i < sum3; i++)
	{
		while (Bo[i] != -1 && sum2 > 0)//此被消元行不是全0且实时的被消元行还存在，不为空
