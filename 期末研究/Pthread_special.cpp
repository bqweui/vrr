...
void* Pthread(void* parm)
{
	threadParm_t* p = (threadParm_t*)parm;
	int t_id = p ->threadId;
	for (int i = t_id; i < sum3; i+= thread_num)
	{
		while (Bo[i] != -1 && sum2 > 0)//此被消元行不是全0且实时的被消元行还存在，不为空
		{
			if (Xo[Bo[i]] != -1)//对应的消元子存在
...
int main()
{
	...
	for (int i = 0; i < thread_num; i++)
	{
				...
				pthread_create(&threads[i], NULL, Pthread, (void*)&threadParm[i]);
	}
	for (int i = 0; i < thread_num; i++)
	{
	...
	{
		threadParm[i].threadId = i;
		pthread_create(&threads[i], NULL, Pthread_SSE, (void*)&threadParm[i]);
	}
	for (int i = 0; i < thread_num; i++)
	{
		pthread_join(threads[i], NULL);
	...
	QueryPerformanceCounter((LARGE_INTEGER*)&tail);
	cout << "Pthread_SSE" << " " << (tail - head) * 1000.0 / freq << endl;
	init();
	putX();
	putB();
	QueryPerformanceFrequency((LARGE_INTEGER*)&freq);
	...
	{
		threadParm[i].threadId = i;
		pthread_create(&threads[i], NULL, Pthread_AVX, (void*)&threadParm[i]);
	}
	for (int i = 0; i < thread_num; i++)
	{
		pthread_join(threads[i], NULL);
	...
	QueryPerformanceCounter((LARGE_INTEGER*)&tail);
	cout << "Pthread_AVX" << " " << (tail - head) * 1000.0 / freq << endl;
	print();
	return 0;
}
